# This code does Data cleaning for the Australian vaccine usage.
...
# Read the data file from https://github.com/YouGov-Data/covid-19-tracker.
...
vaccine_cols <- grep("vaccine|vaccin|dose|booster|vac_boost|vac14", names(df1), ignore.case = TRUE, value = TRUE)
...
### EDA ###
# Create a histogram of vaccine desire in Australia.
...
ggplot(df1, aes(x = vaccine_desire, fill = factor(vaccine_desire))) +
  ...
  labs(x = "Vaccine Desire", y = "Count", fill = "Vaccine Desire", title = "Frequency of Vaccine Desire in Australia") +
  theme_minimal()
...
ggplot(df1, aes(x = week_number, fill = factor(vaccine_desire))) +
  geom_bar(position = "dodge") + 
  scale_fill_viridis_d(option = "cividis") +
  labs(x = "Week Number", y = "Count", fill = "Vaccine Desire", title = "Vaccine Desirability in Australia During COVID-19") +
  theme_minimal()

# Save the dateframe.
write_csv(df1, "vac_des_aus.csv")
