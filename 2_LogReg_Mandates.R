# This code creates the logistic regression model for before and after mandate periods
# for face mask behaviour in Australia.
...
set.seed(1826179)
# Read the new data created and set as factor main response variables as well as select only before mandate periods (with '0').
df<-read.csv('./face_mask_ausmand.csv')
df <- df %>% 
  filter(within_mandates %in% c(0))
...
### Model 1 before mandate periods###
...
model1 <- train(
  face_mask_behaviour_binary ~ .- (RecordNo + endtime + face_mask_behaviour_scale
                                   ...
                                   + protective_behaviour_nomask_binary + within_mandates),
  data = df,
  method = "glm",
  family = "binomial",
...
### Model 2 after mandate periods###
df<-read.csv('./face_mask_ausmand.csv')
df <- df %>% 
  filter(within_mandates %in% c(1))
df$face_mask_behaviour_binary <- as.factor(df$face_mask_behaviour_binary)
df$protective_behaviour_binary <- as.factor(df$protective_behaviour_binary)
